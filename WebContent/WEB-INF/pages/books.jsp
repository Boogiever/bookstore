...
<script type="text/javascript">
	$(function(){
		...
		$("#pageNo").change(function(){
			var val=$(this).val();
			val=$.trim(val);
			//1校验val是否为数字1,2，而不是a12,b
			var flag=false;
			var reg=/^\d+$/g;
			var pageNo=0;
			if(reg.test(val)){
			//2,校验val在一个合法的范围内：1——totalPageNumber
			 pageNo=parseInt(val);
			if(pageNo>=1&&val<=parseInt("${bookpage.totalPageNumber}")){
				flag=true;
			}
			}
			if(!flag){
			...
			//3,页面跳转
			var href="/bookstoree/bookServlet?method=getBooks&pageNo="+pageNo+"&"+$(":hidden").serialize();
			window.location.href=href;
...
<body style=" background:url(backImage10.jpg);background-size:100%; background-attachment: scroll">
		
		<h1  align="center" style="color:#0c8918;">欢迎来到购物书城</h1>
